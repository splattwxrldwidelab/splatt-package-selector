...
/**
 * Render Discord preview clip with adaptive resolution/fps/bitrate.
 * 
 * Requirements:
 * - Full duration up to 176s, no shortening
 * - Target ≤8.5MB (hard cap 9MB)
 * - Correct bitrate math in kbps
 * - Adaptive: ≤60s 480p@30fps, 60-120s 480p@24fps, >120s 360p@20fps
 * - H.264 libx264 yuv420p, AAC 48-64kbps, +faststart
 * - maxrate/bufsize constraints
 */
async function renderPreview(job) {
  ...
  if (dur <= 60) {
    // ≤60s: 480p @ 30fps, AAC 64kbps
    resolution = '854:480';
    fps = 30;
    audioBitrate = 64;
  } else if (dur <= 120) {
    // 60-120s: 480p @ 24fps, AAC 56kbps
    resolution = '854:480';
    fps = 24;
    audioBitrate = 56;
  } else {
    // >120s: 360p @ 20fps, AAC 48kbps
    resolution = '640:360';
    fps = 20;
    audioBitrate = 48;
  }
  ...
  // Correct bitrate math in kbps:
  // targetKbps = floor((targetBytes * 8 / duration) / 1000)
  // Target: 8500000 bytes (8.5MB)
  // Audio takes audioBitrate * duration seconds worth
  ...
  // Clamp to safe range to handle edge cases
  videoBitrate = Math.max(200, Math.min(2500, videoBitrate));

  // Constrained bitrate: maxrate and bufsize
  ...
  console.log(
    `[preview] job=${job.id} dur=${dur.toFixed(2)}s res=${resolution} fps=${fps} vbitrate=${videoBitrate}k abitrate=${audioBitrate}k`
  );
 ...
 * Flow: preview_claim -> preview_heartbeat -> render -> preview_prepare_upload -> upload -> preview_complete
 * Respects MAX_RETRIES; after 3 attempts, calls preview_fail and stops (no infinite reclaim loop).
 */
    ...
    while (
      ...
      try {
        ...
        if (
          ...
          // Terminal failure: call preview_fail and stop (prevent runaway reclaims)
          await call(
            'preview_fail',
            {
              job_id:
