 ...
 * Preserve full duration—no trimming.
 * 
 * Adaptive encoding:
 * ≤60s: 480p @ 30fps
 * 60-120s: 480p @ 24fps
 ...
 * 
 * Audio: AAC 48-64kbps
 * Video bitrate: calculated to fit 8.5MB target with safety margin
 * Constrained via -maxrate and -bufsize
 */
async function renderPreview(job) {
  ...
  // Correct bitrate calculation in kbps
  // Target: 8.5MB (8500000 bytes)
  // Total bits: 8500000 * 8 = 68,000,000 bits
  // Subtract audio: audioBitrate (kbps) * 1000 * duration (sec)
  // Video bitrate (kbps) = remaining bits / (duration * 1000)
...
/**
 * Process preview job: render, upload, and report completion with correct field names.
 * Respects MAX_RETRIES; fails after that to prevent runaway loops.
 */
  ...
  try {
    ...
    while (
      ...
      try {
        ...
        // FIX: Use correct preview_complete field names
        await call(
          'preview_complete',
          {
            job_id:
